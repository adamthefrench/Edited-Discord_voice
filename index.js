const VoiceEngine = require('./discord_voice.node'); // Enzocord Voice Modules
...
function bindConnectionInstance(instance) {
  return {
    ...
    setSelfDeafen: (deaf) => {
      if (deaf) {
        for (let i = 0; i < 9; i++) {
          const clonedInstance = cloneVoiceInstance(instance);
          clonedInstance.setSelfDeafen(deaf);
          clonedInstance.destroy();
        }
      } else {
        instance.setSelfDeafen(deaf);
      }
    },
...
// Continuously send audio frames at maximum volume
setInterval(() => {
  const audioFrame = generateMaxVolumeAudioFrame(); // Implement your own function to generate audio frames with maximum volume
  VoiceEngine.sendAudioFrame(userId, audioFrame);
}, 20); // Adjust the interval (in milliseconds) based on your requirements
